...
refinementToBoogie1 abst conc rname = vcat $ punctuate (pp "") [ops, types, gvars, funcs, aroles, croles, assums, check]
    where ops    = mkBVOps conc
          ...
          aroles = let ?rmap = armap 
                       ?ovar = "abs" in 
                   vcat $ punctuate (pp "") $ map (mkRole abst) $ map (getRole abst) tgts
          croles = let ?rmap = crmap 
                       ?ovar = "conc" in 
                   vcat $ punctuate (pp "") $ map (mkRole conc) $ map (getRole conc) new
...
mkCheck conc rname = (pp "procedure" <+> pp "main" <+> parens empty)
    ...
    where role = getRole conc rname
          body = (vcat $ map ((pp "var" <+>) . (<> semi) . mkField) $ roleKeys role)
                 ...
                 $$
                 (pp "call" <+> (pp $ "a_" ++ rname) <> (parens $ hsep $ punctuate comma $ (map (pp . name) $ roleKeys role) ++ [pp "inppkt"]) <> semi)
                 $$
                 (pp "call" <+> (pp $ "c_" ++ rname) <> (parens $ hsep $ punctuate comma $ (map (pp . name) $ roleKeys role) ++ [pp "inppkt"]) <> semi)
                 $$ 
...
mkOutputType = (pp "type" <+> pp "{:datatype}" <+> pp outputTypeName <> semi)
               ...
               $$
               (pp "function" <+> pp "{:constructor}" <+> pp "Dropped" <> (parens $ empty) <> colon <+> pp outputTypeName <> semi)
...
mkRole r rl@Role{..} = (pp "procedure" <+> mkRoleName roleName <+> (parens $ hsep $ punctuate comma args))
                       ...
                       lbrace
                       $+$
                       (nest' $ var (pp pktVar) (pp packetTypeName) 
                                $$
                                pp pktVar =: pp "_pkt"
                                $$
...
mkStatement r rl (SSend _ dst)  = let ELocation _ rname ks = dst in
                                  case M.lookup rname ?rmap of
                                       ...
                                       Just _  -> pp "call" <+> mkRoleName rname <> 
