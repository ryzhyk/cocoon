...
\textrm{argument} & arg & = & \tau~id \\
                  & args & = & arg \mid arg, args \\
\textrm{fields} & \textit{fields} & = & \tau~id \mid \tau~id, fs \\
...
\textrm{type specs} & \tau & = & \texttt{uint <} n \texttt{>}
                                 ...
                                 \mid \texttt{struct \{} \textit{fields} \texttt{\}} \\
\textrm{statement} & a & = & \texttt{filter}~e \mid \texttt{?send $id$} \mid \texttt{?send $id$ [$e$]} \mid \texttt{send $d$} \\
                   &   &   & \mid e_3 \texttt{:=} e_2 \mid \texttt{if $e$ then $s_1$} \mid \texttt{if $e$ then $a_1$ else $a_2$} \\
