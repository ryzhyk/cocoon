...
$\sem{a}pkt,env :: \mathcal{P}(pkt,env)$\qquad(only nondeterminism)

$\sem{\texttt{filter}\ e}pkt,env=\{(pkt,env)\mid\sem{e}pkt,env=\texttt{true}\}$
\qquad(could be $\emptyset$)

$\sem{e_3:=e_2}pkt,env=\{(pkt[\sem{e_3}\mapsto\sem{e_2}],env)\}$\qquad
(singleton, TODO be more precise on $\sem{e_3}$)

$\sem{\texttt{if $e$ then $a_1$}}pkt,env=
\sem{a_1}pkt,env$ if $\sem{e}pkt,env=\texttt{true}$

$\sem{\texttt{if $e$ then $a_1$}}pkt,env=
\{(pkt,env)\}$ if $\sem{e}pkt,env=\texttt{false}$

$\sem{\texttt{if $e$ then $a_1$}}pkt,env=
\texttt{ERROR}$ otherwise

$\sem{\texttt{if $e$ then $a_1$ else $a_2$}}pkt,env=
\sem{a_1}pkt,env$ if $\sem{e}pkt,env=\texttt{true}$

$\sem{\texttt{if $e$ then $a_1$ else $a_2$}}pkt,env=
\sem{a_2}pkt,env$ if $\sem{e}pkt,env=\texttt{false}$

$\sem{\texttt{if $e$ then $a_1$ else $a_2$}}pkt,env=
\texttt{ERROR}$ otherwise

$\sem{\texttt{havoc } e}pkt,env=\{pkt[\sem{e}\mapsto v],env\mid\textrm{$v$ can be any value}\}$
\qquad(Shall we constrain $v$ of being the right type? Usually bad idea to mix typing and
semantics. TODO be more precise on $\sem{e}$)

$\sem{\texttt{assume}\ e}pkt,env=\{(pkt,env)\mid\sem{e}pkt,env=\texttt{true}\}$
\qquad(cannot be $\emptyset$. But then WHAT?)

$\sem{\texttt{let}\ id=e}pkt,env=\{(pkt,env[id\mapsto\sem{e}])\}$ if $id\not\in{\sf dom}(env)$
\qquad($env$ is extended)

\subsection{Final statement}
$\sem{f}pkt,env :: \mathcal{P}(\mathcal{P}(pkt,env))$
\qquad(both nondeterminism and sending out several packets)

$\sem{\texttt{send }e}(pkt,env)=\sem{e}(pkt,env)$\qquad(role seen as a function)

$\sem{\texttt{?send }id[e]}(pkt,env)=\bigcup\left(\sem{id(args)}(pkt,env)\mid\sem{e}args=\texttt{true}\right)$\qquad(role seen as a function; need to be made more precise)

$\sem{\texttt{fork}}$

Sending is like calling a function, the function being the role we are sending to.

Q: But does the environment set up by the let carry over? Probably not.

Q: Do we want the semantics to keep the trace of the nodes the packet has been through?
Probably not if we want to keep refinement as the inclusion of semantics.
