...
\\
\sem{\texttt{function }id(args)} & = 
\left({\sf Val}^{({\sf Pkt}\times{\sf Env})}\right)^{\sf Val}
&& 
\text{(for a declaration only,) semantics is set of all possible functions}\\
\sem{\texttt{function }id(args)=e} & = 
\{\lambda args.\lambda (\pkt,\env).\sem{e}(\pkt,\env)\}
&& \text{
...
\paragraph{Remark} The definition of the semantics of \texttt{role} imposes that the 
