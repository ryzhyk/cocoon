...
Syntactic sugar: $\texttt{?send }id[e]\equiv
\texttt{let }args=\texttt{havoc};\ \texttt{assume }e;\ \texttt{send }id[args];$
...
$\sem{\texttt{?send }id[e]}(\pkt,\env)=\bigcup\left(\sem{id[args]}(\pkt,\env)\mid\sem{e}\pkt,\env,args=\texttt{true}\right)$\qquad(role seen as a function; need to be made more precise on $args$ (``of the right type''??))

$\sem{\texttt{fork } (args\mid e)\ a}=\bigcup\left(\sem{a}(\pkt,\env[args\mapsto argvals])\mid\sem{e}\pkt,\env[args\mapsto argvals],\texttt{None}=\texttt{true}\right)$
